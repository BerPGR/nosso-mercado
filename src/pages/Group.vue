...
<template>
  <UContainer>
    ...
    <div class="grid md:grid-cols-3 sm:grid-cols-1 gap-10 mt-10 md:mt-0"
      v-if="groupOpenListas.length > 0 && loading === false">
      <UCard v-for="lista in groupOpenListas" :key="lista.title" class="bg-green-50 shadow-2xl">
        <template #header>
    ...
    <UPageCard spotlight title="Ops!" description="Parece que voce ainda nao possui listas"
      v-if="loading === false && groupClosedLists.length === 0" class="md:w-1/3 mx-auto mt-10">
      <img src="@/assets/carrinho.png" />
    </UPageCard>

    <h1 class="text-4xl font-bold mb-10 mt-20">Listas Feitas</h1>
    <div class="mt-20" v-if="groupClosedLists.length > 0">
      <div class="grid md:grid-cols-3 sm:grid-cols-1 gap-10 my-10" v-if="groupClosedLists.length > 0">
        <UCard v-for="lista in groupClosedLists" :key="lista.id" class="bg-green-50 shadow-2xl">
          <template #header>
            <div class="flex items-center justify-between">
              <h1 class="text-xl font-semibold">{{ lista.title }}</h1>
              <ModalDeleteList :id="lista.id" @delete="deleteList" />
    ...
    <UPageCard spotlight title="Ops!" description="Parece que você ainda não possui listas"
      v-if="loading === false && groupClosedLists.length === 0" class="md:w-1/3 mx-auto mt-10">
      <img src="@/assets/carrinho.png" />
    </UPageCard>
  </UContainer>
...
const { getGroupOpenListsFromFirebase, getGroupClosedListsFromFirebase, deleteListById } = useList()

const loading = ref(true)
const groupOpenListas = ref<Lista[]>([])
const groupClosedLists = ref<Lista[]>([])
...
async function loadLists() {
  loading.value = true
  groupOpenListas.value = await getGroupOpenListsFromFirebase(groupId)
  groupClosedLists.value = await getGroupClosedListsFromFirebase(groupId)
  loading.value = false
}
